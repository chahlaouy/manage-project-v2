        ...
        <form wire:submit.prevent="submitForm" class="p-8" x-data="{ attachActive: false, descActive: true }">
            ...
            <div class="" x-show="descActive">
                ...
                @error('general_notes')
                    <span class="text-red-400">{{$message}}</span>
                @enderror
                <button x-on:click="Livewire.emit('ProjectAdded')" type="submit" class="shadow rounded text-white px-12 py-2 bg-purple-600 focus:outline-none hover:bg-purple-700 mr-4">
                    {{__('Create now')}}
                </button>
            </div>
